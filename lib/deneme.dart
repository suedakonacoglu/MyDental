...
class _RandevuEkleState extends State<RandevuEkle> {
  Future<void> getTedavi() async {
    _hastalikData = await _db.getHastalik(tedavi);
    _tedaviController.text = _hastalikData.hastalikAdi;
  }
  ...
  List<String> hastalikIsimleri = ['Seçiniz'];

  void getHastalikIsimleri() {
    FutureBuilder<List<HastalikData>>(
      ...
      builder: (context, snapshot) {
        ...
        if (hastaliklar != null) {
          for (int i = 0; i < hastaliklar.length; i++) {
            final hastalik = hastaliklar[i];
            hastalikIsimleri.add(hastalik.hastalikAdi);
          }
        }
